...
const Login: React.FC = () => {
  const {
    ...
    buttons: { GOOGLE_BUTTON_TEXT, EMAIL_BUTTON_TEXT, SIGNUP_BUTTON_TEXT, FORGOT_PASSWORD_BUTTON_TEXT },
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const destination = response?.onboardingSkipped ? "/create-organization" : "/dashboard"
    ...
    <>
      ...
      <Card className="max-w-md w-full mx-auto p-6 space-y-4">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground text-sm">{LOGIN_PAGE_SUBTITLE}</p>
        </div>
