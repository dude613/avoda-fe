    ...
    () => [
      {
        ...
        icon: (isSelected: boolean) => (
          <FiUser
            className={`text-4xl p-[3px] mb-1 rounded-full transition ${
      ...
      },
      {
        ...
        icon: (isSelected: boolean) => (
          <FiUsers
            className={`text-4xl p-1 mb-1 rounded-full transition ${
              isSelected ? "bg-black text-white" : "bg-gray-100 text-gray-500"
            }`}
          />
        ),
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="mb-8">
          ...
          <p className="text-xs text-textPrimary opacity-70 mb-4 mt-2">
            This will be then name of your organization in the system.
          </p>
        </div>
        ...
        <div className="mt-8">
          ...
          <select
            {...register("industry")}
            className="border text-xs p-3 w-full mb-2 rounded focus:outline-none bg-white focus:ring-1 focus:ring-gray-400"
          >
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-2">
            {companySize.map((size) => (
              <label
                key={size.value}
                className={`box-border rounded-lg p-3 flex flex-col items-center cursor-pointer transition-all ${
                  selectedCompanySize === size.value ? "bg-gray-100": size.bgColor
                } ${
        ...
        <Button
          ...
          iconRight={!loading && <FaArrowRight className="" />}
          disabled={loading}
          className={`${loading ? "opacity-50" : ""} hover:scale-105`}
        />
        <p className="text-xs text-textPrimary opacity-70 mb-4 text-center">
          {`By Creating an organization, you aggree to our Terms and Service and Privacy Policy.`}
        </p>
      </form>
    </div>
  );
